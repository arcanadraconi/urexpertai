...
import { FileText, Copy, ArrowDownToLine, Loader2, Check } from 'lucide-react'
    ...
    <div className="fixed right-0 top-0 h-full w-[800px] bg-[#5b828a]/10 p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button 
            onClick={handleCopy}
            className="p-2 hover:bg-[#1d7f84]/10 rounded-md relative"
      ...
      <div className="h-[calc(100vh-8rem)] overflow-hidden">
        <Card className="h-full bg-white/95 p-6 overflow-y-auto">
          {review && !isReady && (
          ...
          {!review && (
            <div className="flex items-center justify-center h-full text-[#001426]/50">
              Your review will appear here...
            </div>
          )}
          {displayedText && !isEditing && (
            <div className="space-y-4 text-sm text-[#001426]/90 whitespace-pre-wrap">
              {displayedText.split('\n').map((line, index) => {
                if (index === 0) {
                  return <h1 key={index} className="text-2xl font-bold text-[#001426] mb-6">{line}</h1>
                }
          ...
          {isEditing && (
            <textarea
              ...
              className="w-full h-full p-4 text-sm text-[#001426]/90 border rounded-md focus:outline-none focus:ring-2 focus:ring-[#1d7f84]"
